...
Discovery reads the listing formats common gateways publish, but not every endpoint answers in one of them, so treat it as a convenience rather than a guarantee: when it fails or lists nothing, add the model ids by hand and they work just the same. A built-in provider is answered from the installed catalog, even when its base URL points at a gateway, so fetch through a custom provider to see what the gateway really serves. A saved provider can instead be told to ask its own endpoint: set `discoverFromEndpoint: true` under that provider in `$DSH_HOME/profiles/<profile>/cordis.patch.yml`, and **Fetch available models** queries the provider's real listing — including models the installed catalog does not ship yet. A listed model the installed catalog still describes keeps the capacities that entry records; one it does not describe arrives with the protocol and endpoint the fetch used, and the provider's default context-window and output-cap values serve it until you fill them in.
