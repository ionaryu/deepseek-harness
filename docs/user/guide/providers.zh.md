...
提供方保存的模型是当时 pi-ai 目录的快照。pi-ai 升级改变了提供方目录后，新目录不再描述的已保存模型会停止被服务：提供方中该行消失、日志点名该模型，而新目录仍能描述的其他模型继续工作。保存的 `settings.yaml` 行不会被改动，因此后来的 pi-ai 版本再次描述该模型时，它会在下一次读取时自动回归。若想改采当前目录，请打开该提供方，再次选择**获取可用模型**并保存结果。保存的模型全部不在新目录中的提供方会被拒绝，直到其列表得到更新，因为没有任何可服务模型的提供方无法提供服务。
