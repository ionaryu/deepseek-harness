...
A saved `models` list is a snapshot of the pi-ai catalog it was taken against. When the installed catalog does not describe an id the snapshot lists, resolution drops that entry instead of disabling the route: the adapter logs the drop once per configuration — naming the route, the dropped ids, and the remediation — and the models the installed release still describes keep serving. The saved line is left untouched, so an id a future pi-ai release describes again serves on the next resolve. A list whose every id the installed catalog does not describe still refuses, because a route with nothing to serve is a configuration error, never a silent empty route.
...
- **A route's model list is a snapshot, never refreshed automatically** — the served catalog is whatever `settings.yaml` says, and nothing here queries a provider for the models it serves. After a pi-ai upgrade changes a provider's catalog, re-fetch models on the Models page to adopt the new ids; until then, a saved id the installed catalog does not describe is dropped from the route with a logged diagnostic — never deleted from the settings document — and returns automatically when a future release describes it again.
