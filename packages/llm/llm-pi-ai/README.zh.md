...
保存的 `models` 列表是针对当时 pi-ai 目录拍摄的快照。当已安装目录不描述快照点名的某个 id 时，解析会丢弃该条目而不是禁用整条路由：适配器对每次配置只记一条日志——点名路由、被丢弃的 id 与补救办法——已安装版本仍能描述的模型继续服务。保存的那一行不会被改动，因此未来 pi-ai 版本再次描述该 id 时，它会在下一次解析自动回归。列表中的 id 全部不被已安装目录描述时仍然拒绝，因为没有任何模型可服务的路由是配置错误，绝不是一条静默的空路由。
...
profile 通过可选 settings seam 每次操作重新读取：base 与用户的 `llm-pi-ai:` 设置分节按提供方合并，因此用户可以新增路由、覆盖组合路由的一个字段或把路由指向另一个代理，全部在下一个请求生效、无需重启。适配器无法服务的分节会在写入处被拒绝——`settings.mutate` 回答 `settings-rejected`——之后失效的已存储分节会保留 namespace 最后有效值。保存的模型 id 不被已安装目录描述是唯一被容忍的原因：它会被丢弃出路由并记录一条诊断日志，而不是让整个分节失败，因此针对旧 pi-ai 版本保存的文档不可能把整个提供方带离线。当路由集合或某路由的重试策略变化时，插件会原子地重新注册：冲突路由会让此前路由继续服务。
...
- **路由的模型列表是快照，不会自动刷新**——服务的目录就是 `settings.yaml` 的内容；这里没有任何机制向提供方查询它提供的模型。pi-ai 升级改变提供方目录后，请在 Models 页重新获取模型以采纳新 id；在那之前，已安装目录不描述的已保存 id 会带着一条诊断日志被丢弃出路由——绝不会从设置文档中删除——当未来版本再次描述它时自动回归。
